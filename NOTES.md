...
| D15 | **PIECE PLAN REVISED (ahl 2026-09-13, supersedes D2/G11 piece-hood):** the model = ONE SURROUND piece + THREE removable pieces (Mountains, Valley, Desert). The surround is a 3-color AMS multi-body print: gray non-CA land + continuous water + yellow coastal band, with the Channel Islands as yellow islands ON the surround (islands piece SCRAPPED). Coast is not removable — it is printed into the surround. Multi-body 3MF path already validated (NOTES-3mf.md). | |
